...
/**
 * **Heurística Aleatoria Mejorada con Refinamiento**
 * - Inicializa con un enfoque voraz (greedy).
 * - Realiza swaps inteligentes para mejorar la partición.
 * - Utiliza refinamiento iterativo para encontrar mejores soluciones.
...
int improvedHeuristic(vector<int> weights, int maxIterations) {
    ...
    // **Paso 1: Inicialización Greedy (Asignación voraz)**
    sort(weights.rbegin(), weights.rend()); // Ordenamos pesos de mayor a menor
    int sum1 = 0, sum2 = 0;
    ...
    for (int iter = 0; iter < maxIterations; iter++) {
        ...
        if (i != j && assignment[i] != assignment[j]) {
            swap(assignment[i], assignment[j]);  // Intercambiamos entre los conjuntos
            int newDiff = calculateDifference(weights, assignment);

            if (newDiff < bestDiff) {
                bestDiff = newDiff;
            } else {
                swap(assignment[i], assignment[j]);  // Revertir si no mejora
            }
        }
...
int main() {
    ...
    cin >> maxWeight;
    cout << "Seleccione el algoritmo (E: Exhaustiva, B: Branch and Bound, H: Heurística Aleatoria, I: Heurística Mejorada): ";
    cin >> algorithmChoice;
